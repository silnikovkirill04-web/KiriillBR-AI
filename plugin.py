...
DEFAULT_PROMPT = (
    ...
    "ЦЕНООБРАЗОВАНИЕ И РАСЧЁТ:\n"
    "- В лоте цена может быть указана ЗА ЕДИНИЦУ объёма (например «75₽ за 100 MMR», «500₽ за 1000 голосов», «300₽ / месяц»). "
    "Ищи это в поле Цена, в названии, описании или игровых параметрах.\n"
    "- Если покупатель спрашивает про объём, ОТЛИЧНЫЙ от единицы, — считай ПРОПОРЦИОНАЛЬНО: "
    "Итог = (Запрошенный_объём / Единица_лота) × Цена_за_единицу.\n"
    "- Пример: лот «75₽ за 100 MMR», покупатель просит 300 MMR → 300 / 100 = 3 → 3 × 75 = 225₽. "
    "В ответе покажи расчёт: «75₽ × 3 = 225₽ за 300 MMR».\n"
    "- Если покупатель предлагает СВОЮ цену (например «сможешь за 200₽ 300 MMR?») — сначала посчитай цену по лоту, "
    "назови её, а затем скажи, что скидка/торг на усмотрение продавца и ты передал запрос продавцу.\n"
    "- НИКОГДА не пиши «скидка не предусмотрена», «торг не предусмотрен», «скидок нет» — этих данных в лоте нет. "
    "Цена в лоте — это базовая цена; про скидки говорит только продавец.\n"
    "- Цена, указанная в лоте, — это сумма, которую получает продавец (без комиссии FunPay). "
    "Итоговая цена для покупателя в интерфейсе FunPay может быть выше за счёт комиссии площадки. "
    "Если покупатель спрашивает точную сумму к оплате — скажи, что итог с комиссией FunPay отображается "
    ...
    "«торг не предусмотрен». Просто назови цену по лоту и укажи, что скидка на усмотрение продавца.\n"
...
DEFAULTS = {
    "version": 14, "enabled": True, "setup_done": False,
...
def _clean_ai_answer(text: str) -> str:
    ...
    Не блокирует сообщение, просто правит неудачную формулировку."""
    result = str(text or "")
...
def _sys_prompt(lot: dict[str, Any] | None, full_chat: bool,
    ...
    return (
        ...
        f"{promises}{extra}\n"
        "ЦЕНООБРАЗОВАНИЕ И РАСЧЁТ:\n"
        "- Если в лоте указано, что цена дана ЗА ЕДИНИЦУ объёма (например «за 100 MMR», «за 1000 голосов», «за 1 месяц»), "
        "то при вопросе о другом количестве считай ПРОПОРЦИОНАЛЬНО: "
        "Итог = (Запрошенный_объём / Единица_лота) × Цена_за_единицу. Показывай расчёт.\n"
        "- Пример: 75₽ за 100 MMR → 300 MMR = 3 × 75 = 225₽. В ответе пиши «75₽ × 3 = 225₽ за 300 MMR».\n"
        "- Если покупатель предлагает свою цену («сможешь за X руб Y единиц?») — сначала посчитай и назови "
        "цену по лоту, затем скажи, что скидка/торг на усмотрение продавца и ты передал запрос продавцу.\n"
        "- НИКОГДА не пиши «скидка не предусмотрена», «торг не предусмотрен», «скидок нет» — этих данных в лоте нет.\n"
        "- Цена в лоте — это сумма продавцу (без комиссии FunPay). Итоговая для покупателя может быть выше "
        "из-за комиссии площадки; точная сумма видна при оформлении заказа.\n\n"
        "Дополнительно:\n"
