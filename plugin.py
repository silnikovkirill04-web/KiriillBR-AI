    ...
    try:
        ...
        for cid, hist in (data.get("chats") or {}).items():
            ...
            for item in hist[-_HISTORY_HARD_CAP:]:
                ...
                if rl in ("user", "assistant", "system_note") and ct:
...
def _load_lot_stock():
    ...
    try:
        ...
        with LOCK:
            ...
            LOT_STOCK_META = m2
        logger.info("stock: %d лотов, %d позиций", len(LOT_STOCK),
                    sum(len(v["items"]) for v in LOT_STOCK.values()))
...
def _stock_add(lot_id, text, ts=None):
    ...
    with LOCK:
        ...
        if len(rec["items"]) >= _STOCK_MAX_ITEMS_PER_LOT: return False
...
def _stock_clear(lot_id):
    lid = str(lot_id or "").strip()
    with LOCK:
        if lid in LOT_STOCK: LOT_STOCK.pop(lid, None)
    _save_lot_stock()
    return True
...
def save_orders_worker(c):
    ...
    while not STOP.is_set():
        try:
            ...
            _save_lot_stock(); _save_delivered()
        except Exception: pass
...
def _find_lot_for_order(c, order):
    for attr in ("lot_id", "offer_id"):
        ...
        if lid:
            ...
            with LOCK: lot = LOTS.get(lid_str)
            if lot: return lot
            try:
...
def _ensure_lot(c, lid):
    ...
    try:
        ...
        base = {"id": lid_str,
            ...
            "full_description": "",
            "price": getattr(f, "price", None), "currency": "",
            ...
            "image_urls": []}
        with LOCK: LOTS.setdefault(lid_str, base)
        return True
    except Exception as e:
...
def _auto_deliver_from_stock(c, order, lot, order_id, chat_id, buyer_name, title):
    ...
    if meta.get("auto") is False: return False
    count = _stock_count(lid)
    if count <= 0: return False
    item_text, remaining = _stock_pop(lid)
    if not item_text: return False
...
def _fulfill_paid_order(c, order):
    ...
    if SETTINGS.get("auto_delivery_from_stock", True):
        if _auto_deliver_from_stock(c, order, lot, order_id, chat_id, buyer_name, title): return
...
def _compress_history_item(item, is_old=False):
    ...
    if len(content) > cap: content = content[:cap].rstrip() + "…"
...
def ask_ai(m, buyer_text, lot):
    ...
    if first: first = first.replace("\r", "")
    if SETTINGS.get("strip_safety_junk", True) and first:
        for _ in range(10):
            try: new = _RE_SAFETY_JUNK.sub("", first).strip()
            ...
            if results:
                ...
                try:
                    ...
                    if final:
                        ...
                        if SETTINGS.get("strip_safety_junk", True):
                            for _ in range(10):
                                try: new = _RE_SAFETY_JUNK.sub("", final).strip()
                                except Exception: break
                                if new == final or not new: break
                                final = new
                        if not final or not final.strip(): final = first
...
def init_telegram(cardinal):
    ...
    def main_kb():
        ...
        kb.row(B("⚙️ Прочее", callback_data=f"{CB}:m:misc"),
               B("🔄 Обновления", callback_data=f"{CB}:m:update"))
    ...
    def show_stock_menu(call):
        ...
        if stock_lots:
            ...
            for lid, rec in stock_lots[:10]:
                ...
                with LOCK: auto_mark = "🤖" if (LOT_STOCK_META.get(lid) or {}).get("auto", True) else "🚫"
    ...
    def ask_stock_view(call):
        msg = bot.send_message(call.message.chat.id,
            "👁️ Пришлите <code>lot_id</code> или <code>all</code>.",
            reply_markup=CLEAR_STATE_BTN())
    ...
    def set_stock_view(m):
        ...
        with LOCK: lot = dict(LOTS.get(lid) or {})
        with LOCK: meta = dict(LOT_STOCK_META.get(lid) or {})
        if not rec:
    ...
    def bootstrap_toggle(call):
        SETTINGS["bootstrap_history"] = not bool(SETTINGS.get("bootstrap_history", True))
        save_config()
        try: bot.answer_callback_query(call.id, f"Bootstrap: {'вкл' if SETTINGS['bootstrap_history'] else 'выкл'}")
        except Exception: pass
        show_lots_menu(call)
    ...
    def show_lot_instr(call):
        ...
        kb.row(B("➕", callback_data=f"{CB}:lins:add"), B("➖", callback_data=f"{CB}:lins:del"),
               B("◀️", callback_data=f"{CB}:m:lots"))
        show_text(call, text, back_cb=f"{CB}:m:lots", kb_override=kb)
    ...
    tg.cbq_handler(bootstrap_toggle, lambda c: c.data == f"{CB}:bootstrap")
